...
async function initDb(c) {
    await c.query(`
        ...
        CREATE TABLE IF NOT EXISTS timetable (id VARCHAR(100) PRIMARY KEY, username VARCHAR(100) NOT NULL, day INTEGER NOT NULL, date VARCHAR(50), subject VARCHAR(100) NOT NULL, start_time VARCHAR(10) NOT NULL, end_time VARCHAR(10) NOT NULL, lesson TEXT);
            ...
            if (useDb) {
                ...
                try {
                    ...
                    res.status(200).json({
                        ...
                        timetable: tt.rows.map(r => ({ id: r.id, username: r.username, day: r.day, date: r.date, subject: r.subject, startTime: r.start_time, endTime: r.end_time, lesson: r.lesson })),
                ...
                try {
                    ...
                    try {
                        if (key === 'users') {
                            ...
                            for (const t of data) await c.query('INSERT INTO timetable VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [t.id, t.username, t.day, t.date, t.subject, t.startTime, t.endTime, t.lesson]);
                        } else if (key === 'logs') {
